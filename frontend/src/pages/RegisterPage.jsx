...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (res.status == 400)
                    setError("Внесените податоци се невалидни или кодот за верификација е погрешен. Провери ги полињата.");
    ...
    const handleSendCode = async () => {
        ...
        try {
            const res = await fetch(`${BACKEND_URL}/api/auth/register/send-code`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            if (!res.ok) {
                throw new Error("Не успеавме да го испратиме кодот. Обиди се повторно.");
            }

            setCodeSent(true);
            setCooldown(30); // Започни одбројување од 30 секунди
        } catch (err) {
                    ...
                    <form className="space-y-5" onSubmit={handleSubmit}>
                        <div className="space-y-2">
                            ...
                            <div className="relative mt-2">
                                <Mail
                                    className="absolute w-4 h-4 -translate-y-1/2 left-3 top-1/2 text-muted-foreground"></Mail>
                                <input
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative flex items-center mt-2">
                                ...
                                <button
                                    ...
                                    className="absolute right-1 h-8 px-3 text-xs font-medium bg-background border border-input rounded-sm hover:bg-accent hover:text-accent-foreground text-foreground transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {isSendingCode
                                        ? "Испраќање..."
                                        : cooldown > 0
                        ...
                        {codeSent && (
                            <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                                ...
                                <div className="relative mt-2">
                                    <Key
                                        className="absolute w-4 h-4 -translate-y-1/2 left-3 top-1/2 text-muted-foreground"></Key>
                                    <input
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative mt-2">
                                <Lock
                                    className="absolute w-4 h-4 -translate-y-1/2 left-3 top-1/2 text-muted-foreground"></Lock>
                                <input
                                    className="flex w-full h-10 px-3 py-2 pl-10 text-base border rounded-md border-input bg-background ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm"
                                    id="password"
                                    value={form.password}
                        ...
                        {error && (
                            <div
                                className="flex items-center gap-2 px-4 py-3 text-sm text-red-700 border border-red-200 rounded-lg bg-red-50">
